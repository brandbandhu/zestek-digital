...
import { fadeInLeft, fadeInRight, fadeUp, staggerContainer } from "@/lib/motion";
        ...
        <div className="absolute inset-0 z-[2] flex items-start">
          <div className="container mx-auto px-4 sm:px-6">
            <motion.div
              ...
              initial="hidden"
              animate="show"
              className="max-w-3xl pt-20 text-[#101828] md:pt-24 lg:pt-28"
            >
              <motion.h1 variants={fadeInLeft} className={`mt-4 text-3xl font-extrabold leading-tight md:text-4xl lg:text-5xl ${overlay.titleClassName ?? ""}`}>
                {overlay.title}
              </motion.h1>
              <motion.p variants={fadeUp} className="mt-4 max-w-2xl text-sm leading-7 text-[#101828]/88 md:text-lg md:leading-8">
                {overlay.body}
              </motion.p>
            </motion.div>
          </div>
      ...
      <div className="absolute inset-0 z-[1] bg-gradient-to-r from-white/90 via-white/62 to-transparent" />
        <div className="absolute inset-0 z-[2] flex items-center">
          <div className="container mx-auto px-4 sm:px-6">
            ...
            >
              <motion.span variants={fadeUp} className="inline-flex items-center rounded-full bg-white/90 px-3 py-1 text-xs font-semibold uppercase tracking-[0.28em] text-navy">
                {overlay.label}
              </motion.span>
              <motion.h1 variants={fadeInLeft} className="mt-4 text-3xl font-extrabold leading-tight md:text-4xl lg:text-5xl">
                {overlay.title}
              </motion.h1>
              <motion.p variants={fadeUp} className="mt-4 max-w-2xl text-sm leading-7 text-foreground/80 md:text-base md:leading-8">
                {overlay.body}
              </motion.p>
    ...
    <>
      ...
      <div className="absolute inset-x-0 top-0 z-[2] px-4 pt-7">
        <motion.div
          ...
          initial="hidden"
          animate="show"
          className="max-w-[18rem] text-navy"
        >
          <motion.span variants={fadeUp} className="inline-flex items-center rounded-full bg-white/90 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.24em] text-navy">
            {overlay.label}
          </motion.span>
          <motion.h1 variants={fadeInLeft} className="mt-3 text-2xl font-extrabold leading-tight">{overlay.title}</motion.h1>
          <motion.p variants={fadeUp} className="mt-3 text-xs leading-6 text-foreground/80">{overlay.body}</motion.p>
